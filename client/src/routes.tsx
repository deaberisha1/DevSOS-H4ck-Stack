...
import { EventSessionContext } from "./hooks/useEvent";
    ...
    <section className={css.section}>
      <div className={css.container}>
        ...
        <p className={css.sub} style={{ maxWidth: "52ch" }}>
          The link may be out of date, or the event it pointed at has closed. If
          you have an event code, you can join from here.
        </p>
  ...
  if (!session) {
    return (
      <Navigate
        to="/join"
        replace
        state={{ from: location.pathname, reason: "signed-out" }}
      />
    );
  }
...
function EventIndexRedirect() {
  ...
  return session ? (
    <Navigate to={homePathFor(session)} replace />
  ) : (
...
export default function AppRoutes() {
  return (
    <Routes>
      ...
      <Route
        element={<RequireRole allow={["PARTICIPANT", "MENTOR", "ORGANIZER"]} />}
      >
        <Route element={<EventLayout />}>
          <Route path="/event/:e/requests/:r" element={<RequestDetails />} />
          ...
          <Route
            element={
              <RequireRole allow={["PARTICIPANT", "MENTOR", "ORGANIZER"]} />
            }
          >
